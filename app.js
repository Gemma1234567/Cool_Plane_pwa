...
// adsb.lol returns aircraft as objects with named fields - much nicer than OpenSky's arrays!
// Each aircraft object has: hex, flight, lat, lon, alt_baro, gs (ground speed), track, etc.
function updateMapMarkers(aircraft) {
  ...
  for (const ac of aircraft) {
    ...
    if (planeMarkers[callsign]) {
      ...
      marker.on("click", async () => {
        const type = await getAircraftType(ac.hex || "");
        const dist = userLocation ? Math.round(getDistanceKm(userLocation.lat, userLocation.lon, latitude, longitude)) : "?";
        const alt  = ac.alt_baro ? Math.round(ac.alt_baro) + "ft" : "unknown";
        const spd  = ac.gs ? Math.round(ac.gs) + "kts" : "unknown";
        marker.setPopupContent(`<b>${callsign}</b><br>${type}<br>${dist}km away<br>${alt} · ${spd}`).openPopup();
      });
  ...
  // Remove markers for planes that have left
...
async function sendEmailAlert(callsign, aircraftType, distance, altFt, speedKts, etaString) {
...
async function checkForPlanes() {
  ...
  try {
    // adsb.lol has CORS enabled and is free with no API key needed!
    const url = `https://api.adsb.lol/v2/lat/${userLocation.lat}/lon/${userLocation.lon}/dist/${Math.round(radius)}`;
    const response = await fetch(url);
    if (!response.ok) return;

    const json = await response.json();
    // adsb.lol returns { ac: [...aircraft] }
    const aircraft = json.ac || [];
    console.log(`Found ${aircraft.length} planes`);

    updateMapMarkers(aircraft);

    for (const ac of aircraft) {
      const icao     = (ac.hex || "").trim();
      const callsign = (ac.flight || ac.hex || "").trim();
      const latitude  = ac.lat;
      const longitude = ac.lon;
      const altitude  = ac.alt_baro;  // already in feet!
      const speedKts  = ac.gs;        // ground speed in knots
      const trueTrack = ac.track;
      ...
      if (distance <= radius) {
        ...
        const altFt    = altitude ? Math.round(altitude) + "ft" : "unknown";
        // Convert knots to km/h for ETA calc
        const speedKmh = speedKts ? speedKts * 1.852 : null;
        ...
        await sendEmailAlert(callsign, aircraftType, distance, altFt, speedKts ? Math.round(speedKts) + "kts" : "unknown", etaString);
